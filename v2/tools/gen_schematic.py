...
AgentDeck V2 — wireless 20-key (4x5) AI control deck (ESP32-S3 BLE+USB, OLED, joystick, LiPo).
...
PINOUT (ESP32-S3-WROOM-1-N8R2, module pin -> GPIO -> net):
  ...
  COL0..COL4   GPIO10..14       (p18,19,20,21,22)  matrix column senses
  ...
  JOY_UP/LEFT/DOWN/RIGHT  GPIO15/16/17/18 (p8/9/10/11)  Alps SKQUCAA010 5-way, COM->GND
  JOY_CTR      GPIO1            (p39)      joystick centre push
...
NC = "/Users/barakaeli/Open Source Hardware/NeuralCard/hardware"
...
def mini_symbol(name, desc, body_rect, pins):
    """Minimal V2 symbol. pins: (num, name, x, y, rot)."""
...
MINI_SYMS = [
    mini_symbol("TP4056", "LiPo charger 1A ESOP-8 (docs/datasheets/v2/tp4056.pdf)",
                (-7.62, -7.62, 7.62, 7.62), [
                    ("4", "VCC", -10.16, 5.08, 0),
                    ("8", "CE", -10.16, 2.54, 0),
                    ("1", "TEMP", -10.16, 0, 0),
...
def write_v2_lib():
    ...
    # NeuralCard's module symbol names pin 8 "CE"; it is IO15 on the WROOM-1
    blocks = [b.replace('(name "CE"', '(name "IO15"') if "ESP32-S3-WROOM-1" in b else b
              for b in blocks]
    blocks += MINI_SYMS
    body = '\n'.join(blocks)
...
LIBSYMS = {
    ...
    "V2:OLED_HS96L03": (V2_LIB, "OLED_HS96L03"),
...
PIN_XY = {
    ...
    "V2:OLED_HS96L03": {"1": (-7.62, 3.81), "2": (-7.62, 1.27),
...
# ref -> footprint (lib:fp).  V2:* footprints are drawn in Phase 2.
FP = {
    "U1": "V2:WIRELM-SMD_ESP32-S3-WROOM-1",
...
def section_power():
    ...
    # --- PWR_FLAG: GND only (power_in nets need a driver) ---
    fy = 33.0
    pwr("GND", 340.36, snap(fy) + 2.54)
    wire(340.36, snap(fy) + 2.54, 340.36, snap(fy))
    pwr_flag_at(340.36, snap(fy))
    # VBUS is driven by J1 (power output) and +3V3 by U5 VOUT (power output):
    # no PWR_FLAG on those rails (KiCad 10 ERC flags output-vs-output).
...
ESP_SPECS = {
    ...
    "8": ("lbl", "JOY_UP"), "9": ("lbl", "JOY_LEFT"), "10": ("lbl", "JOY_DOWN"), "11": ("lbl", "JOY_RIGHT"),
    ...
    "38": ("lbl", "VBAT_SNS"), "39": ("lbl", "JOY_CTR"),
    "40": ("gnd",), "41": ("gnd",),
}
...
# ================================================================ SECTION 3
# KEY MATRIX — 4 rows x 5 cols, COL2ROW (switch->anode, cathode->row)
def section_matrix():
    section_box(420, 25, 820, 300, "KEY MATRIX  4x5 COL2ROW  (SW pin1=COL, pin2->D anode, D cathode->ROW)  row0 = system row", 422, 23)
    for idx in range(20):
        r, c = idx // 5, idx % 5
        x = 445.0 + c * 62.0
        y = 55.0 + r * 62.0
...
# ================================================================ SECTION 5
# PERIPHERALS — OLED I2C, 5-way joystick, EC11 encoder
def section_periph():
    section_box(270, 150, 410, 335, "PERIPHERALS  (OLED I2C + SKQUCAA010 joystick + EC11)", 272, 148)

    # OLED module
    ox, oy = 330.0, 180.0
    oled = "V2:OLED_HS96L03"
    ...
    for pn, spec in jspec.items():
        ...
        jlx, _ = PIN_XY[joy][pn]
        tap_dir(spec, px, py, 'L' if jlx < 0 else 'R')
...
\t\t(comment 1 "Wireless AI control deck - 20 keys 4x5, OLED, joystick, 2000mAh LiPo, ESP32-S3 BLE+USB")
\t)
\t(lib_symbols
{lib_symbols}
\t)
