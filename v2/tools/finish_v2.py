...
B = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HW, "AgentDeckV2.kicad_pcb")
SES = sys.argv[1]
...
def build_model(b):
    ...
    segs = {pcbnew.F_Cu: [], pcbnew.In1_Cu: [], pcbnew.In2_Cu: [], pcbnew.B_Cu: []}
    vias, pads = [], []
    for f in b.GetFootprints():
        for p in f.Pads():
            bb = p.GetBoundingBox()
...
# band (B7 feed stub down to its routed run at (60.75,4.66) + A7 jog).
...
n1 = del_box("USB_DM", 59.4, 6.8, 61.1, 8.3)      # A7<->B7 south jog
n2 = del_box("USB_DM", 59.4, 5.4, 61.1, 7.0)      # A7<->B7 north jog
n3 = del_box("USB_DP", 59.0, 5.4, 60.5, 7.0)      # A6<->B6 north jog
add_seg("USB_DM", 60.75, 6.87, 60.75, 4.66)       # B7 feed stub
add_seg("USB_DM", 59.75, 6.87, 59.75, 5.0)        # A7 north stub
add_seg("USB_DM", 59.75, 5.0, 60.75, 5.0)         # A7 -> B7 feed
add_seg("USB_DP", 59.25, 6.87, 59.25, 9.75)       # B6 south descent
...
BESPOKE = [
    ...
    ("VBUS", 0.25,
     [(62.4, 6.87, 62.4, 6.3, pcbnew.F_Cu),
      (62.4, 6.3, 61.9, 5.7, pcbnew.F_Cu),
      (61.9, 5.7, 61.9, 5.3, pcbnew.F_Cu),
      (61.9, 5.3, 61.9, 4.2, pcbnew.B_Cu),
      (61.9, 4.2, 68.5, 4.2, pcbnew.B_Cu),
      (68.5, 4.2, 68.5, 3.9, pcbnew.F_Cu)],
     [(61.9, 5.3), (68.5, 4.2)]),
    # USB_DP pad A6 straight down onto its routed run
    ("USB_DP", 0.2, [(60.25, 6.87, 60.25, 9.75, pcbnew.F_Cu)], []),
]
