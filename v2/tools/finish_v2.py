#!/usr/bin/env python3
"""V2 finish pipeline: import SES, widen power tracks, repair leftover links,
...
Usage (KiCad python):
  .../python3 v2/tools/finish_v2.py <ses_path> [board.kicad_pcb]
...
VSYS_POLY = [(16, 26), (114, 26), (114, 104), (16, 104)]
...
def build_model(b):
    ...
    for t in b.GetTracks():
        if t.GetClass() == "PCB_VIA":
            ...
            s, e = t.GetStart(), t.GetEnd()
            segs.setdefault(t.GetLayer(), []).append(
                (s.x, s.y, e.x, e.y, t.GetWidth() / 2, t.GetNetCode()))
...
def unconnected_pairs():
    ...
    out = os.path.join(tempfile.gettempdir(), "v2_drc.json")
...
# V2: the USB pair (pads, jogs, U3 links) is pre-routed and locked by
# place_pcb.preroute(); del_box skips locked copper, so nothing to rebuild.
print(f"phase4b0: USB jogs untouched (pre-routed, deleted {n1}+{n2}+{n3} stray segs)")
