#!/usr/bin/env python3
"""V4 finish pipeline: import SES, widen power tracks, repair leftover links,
flood GND both layers + VSYS_SW LED-rail pour on B.Cu, stitch vias, fill.
Parameterized port of v3/tools/finish_v3.py (paths, no-via rects, VSYS_SW
pour polygon extended to the V4 audio corner; USB bespoke fixes guarded).

Usage (KiCad python):
  .../python3 v4/tools/finish_v4.py <ses_path> [board.kicad_pcb]
...
NO_VIA_RECTS = [(10.0, 0.0, 30.0, 0.9), (31.0, 8.0, 45.0, 22.0),
                (55.0, 14.0, 57.0, 16.0)]   # antenna, touch void, mic port
# VSYS_SW pour on B.Cu: key field + right-side arm extended down to y=72 so
# it reaches the V4 audio corner (U6 NS4168 + C36/C37 run from VSYS_SW)
VSYS_POLY = [(16, 26), (148, 26), (148, 72), (134, 72), (134, 104), (16, 104)]
...
def unconnected_pairs():
    ...
    out = os.path.join(tempfile.gettempdir(), "v4_drc.json")
...
# V4 guard: freerouting is nondeterministic - only rebuild the USB jogs when
# DRC still reports USB_DP/USB_DM/VBUS leftovers in the connector area
_usb_pairs = [p for p in unconnected_pairs()
              if p[0] in ('USB_DP', 'USB_DM', 'VBUS')
              and 60 < p[1][0] < 92 and p[1][1] < 14]
if not _usb_pairs:
    print('phase4b0: USB area clean - bespoke fixes skipped')
else:
    ...
    # Clearance-guarded, like the BESPOKE block below. The unguarded version
    # drove the USB_DP B6 descent straight through the VBAT_SNS run (a hard
    # short DRC caught only after the pours were poured). The model is built
    # lazily so it reflects the del_box rip-ups above.
    _model = [None]
    ...
    def add_seg(net_name, x1, y1, x2, y2, w=0.2, lay=pcbnew.F_Cu):
        ...
        if net is None:
            return False
        if _model[0] is None:
            _model[0] = build_model(b)
        if not seg_clear(_model[0], mm(x1), mm(y1), mm(x2), mm(y2), lay,
                         net.GetNetCode(), mm(w / 2), m=mm(0.18)):
            print(f"phase4b0: {net_name} seg ({x1},{y1})->({x2},{y2}) "
                  f"BLOCKED - skipped")
            return False
        add_track(b, _model[0], mm(x1), mm(y1), mm(x2), mm(y2), lay, net, mm(w))
        return True
