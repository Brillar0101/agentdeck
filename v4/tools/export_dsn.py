...
Direct port of v3/tools/export_dsn.py (paths + board name changed): exclude
the giant GND net and the VSYS_SW rail (pours by finish_v4.py); CC1/CC2 are
pre-routed by place_pcb.py and fenced off in the routing copy.

Run with KiCad python:  .../python3 v4/tools/export_dsn.py [out.dsn]
"""
import os
import shutil
import sys
...
out = sys.argv[1] if len(sys.argv) > 1 else os.path.join(HW, "ClaudeMicroV4.dsn")
